...
const Home: React.FC = () => {
  ...
  const handleButtonClick = async () => {
    ...
    const url = PLAYWRIGHT_HOST_URI + "/api/screenshot?url=https://haramizu.com";
    ...
    try {
      ...
      const paging = Math.floor(totalItems.widgets[0].total_item / ITEM_NUM) + 1;

      for (let i = 0; i < paging; i++) {
        const contentList: SearchResults = await fetchData(SEARCH_ID, i * ITEM_NUM, ITEM_NUM);
